...
int ExtractDeviatingPixels(uint16_t *eeData, paramsMLX90640 *mlx90640)
{
    ...
    {
        char str[255];
        sprintf(str, "Broken+outlier pixels: %d\n", brokenPixCnt + outlierPixCnt);
        mUART_PutString(str);
        warn = -5;
    } 
    else
    {
        for(pixCnt=0; pixCnt<brokenPixCnt; pixCnt++)
        {
            for(i=pixCnt+1; i<brokenPixCnt; i++)
            {
                ...
                if(warn != 0)
                {
		            mUART_PutString("Broken pixel has adjacent broken pixel\n");
                    return warn;
                }    
            }    
        }
        ...
        for(pixCnt=0; pixCnt<outlierPixCnt; pixCnt++)
        {
            for(i=pixCnt+1; i<outlierPixCnt; i++)
            {
                ...
                if(warn != 0)
                {
		            mUART_PutString("Outlier pixel has adjacent outlier pixel\n");
                    return warn;
                }    
            }    
        } 
        ...
        for(pixCnt=0; pixCnt<brokenPixCnt; pixCnt++)
        {
            for(i=0; i<outlierPixCnt; i++)
            {
                ...
                if(warn != 0)
                {
		            mUART_PutString("Broken pixel has adjacent outlier pixel\n");
                    return warn;
                }    
            }    
        }    
